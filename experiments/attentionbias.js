...
let expName = 'Attention-Bias Modification';  // from the Builder filename that created this script
...
function training_instructionsRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of training_instructionsComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
...
function training_blockRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of training_blockComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
...
function active_conditionRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of active_conditionComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
...
function control_conditionRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of control_conditionComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
